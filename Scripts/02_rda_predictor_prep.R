...
# Load data
inv_df <- 
  readRDS(file.path(pred_dir,"origin_invaded.rds"))
alpha_df <-
  readRDS(file.path(div_dir,"native_alpha.rds"))
lcbd_df <- 
  readRDS(file.path(div_dir,"native_lcbd.rds"))
fishing_df <- 
  read.csv(file.path(pred_dir,"FreshwaterFishing_RecreationDemand.csv"))
hai_df<- 
  read.csv(file.path(pred_dir,"predicted_alteration_region_model.csv"))
...
invd_summary_data <-inv_df %>% 
  mutate(prop_nn = (prov + reg + extr)/tot_sp) 
...
nrow(invd_summary_data[invd_summary_data$reg > 0,])/nrow(invd_summary_data)   
nrow(invd_summary_data[invd_summary_data$extr > 0,])/nrow(invd_summary_data)  
nrow(invd_summary_data[invd_summary_data$prov > 0,])/nrow(invd_summary_data)  
nrow(invd_summary_data[invd_summary_data$prov > 0 |                           
