...
anova(rda_obs, by="term") 
anova(rda_obs, by="margin") 
anova(rda_es, by="term") 
anova(rda_es, by="margin") 
...
plot(rda_obs, type = "n",
     ...
     lwd=1)
abline(h = 0, v = 0, lwd = 2,lty=5)
points(rda_obs, pch=19, display = "sites",cex = 4,col="darkgrey") 
dev.off()
...
obs_varpart <- varpart(
  decostand(response_obs, method = "standardize"),
  obs_inv,
  obs_div,
  obs_ab,
  obs_prop
...
es_varpart <- varpart(
  decostand(response_es, method = "standardize"),
  es_inv,
  es_div,
  es_ab,
  es_prop
